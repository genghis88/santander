...
from sklearn.ensemble import RandomForestClassifier, VotingClassifier, ExtraTreesClassifier, AdaBoostClassifier, GradientBoostingClassifier, RandomTreesEmbedding
...
#emb = RandomTreesEmbedding(n_jobs=-1, n_estimators=1000, min_samples_leaf=100, min_weight_fraction_leaf=0.01, random_state=np.random.RandomState(seed=1352))
#xTrain = emb.fit_transform(xTrain, y)
clf = AdaBoostClassifier(base_estimator=RandomForestClassifier(n_jobs=-1, n_estimators=100, criterion='entropy', max_features=None, min_samples_leaf=100, min_weight_fraction_leaf=0.01, random_state=np.random.RandomState(seed=4731), class_weight='balanced_subsample'), n_estimators=100, learning_rate=0.01, algorithm='SAMME.R')
...
print(scores)
clf.fit(xTrain, y)
#clf = xgb.XGBClassifier(objective='binary:logistic', n_estimators=500, learning_rate=0.02, nthread=4, subsample=0.80, colsample_bytree=0.70, seed=2471)
#scores = cross_validation.cross_val_score(clf, xTrain, y, cv=5, scoring='roc_auc')
#print(scores)
